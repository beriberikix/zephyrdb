...
ZTEST(zephyrdb_samples_verify, test_kv_sample_flow_fails_cleanly_without_backend)
{
	zdb_status_t rc;
	zdb_kv_t kv;
	...
	zassert_equal(rc, ZDB_ERR_INVAL, "expected graceful no-backend failure: %d", rc);

	rc = zdb_deinit(&g_db);
	zassert_equal(rc, ZDB_OK, "deinit failed: %d", rc);
}

ZTEST_SUITE(zephyrdb_samples_verify, NULL, NULL, NULL, NULL, NULL);
